...
class BatchPcapProcessor:
    ...
    def __init__(self, input_pcap: str, labels_file: str, 
                 output_dir: str = "splits", window_size: int = 10,
                 size_limit: str = "1000m", verbose: bool = True):
        """
    ...
    def split_pcap(self) -> List[str]:
        ...
        split_command = [
            "tcpdump", "-r", self.input_pcap, 
            "-w", split_prefix, 
            "-C", self.size_limit[:-1]  # Remove 'm' suffix
        ]
        
        try:
            if self.verbose:
                logger.info(f"Splitting {self.input_pcap} into {self.size_limit} chunks...")
            subprocess.run(split_command, check=True, capture_output=not self.verbose)
            if self.verbose:
                logger.info(f"PCAP file split into chunks in {self.output_dir}")
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"Failed to split PCAP file: {e}")
    ...
    def process_splits(self, split_files: List[str]) -> pd.DataFrame:
        ...
        if self.verbose:
            logger.info(f"Combined {len(combined_df)} total flows from {len(processed_files)} splits")
        
        return combined_df
    
    def _process_single_split(self, split_file: str) -> Optional[str]:
        """Process a single split file using the feature extractor."""
        # Construct command to run feature extraction
        cmd = [
            "python", "-m", "tools.pcap_processing.extractor",
            split_file,
...
Examples:
  ...
  python batch_processor.py file.pcap output.csv --labels flows.csv --no-cleanup
    ...
    parser.add_argument("--size-limit", default="1000m", 
