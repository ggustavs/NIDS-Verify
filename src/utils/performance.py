...
def log_system_info() -> None:
    ...
    logger.info(f"CPU: {info['cpu_count_physical']} physical, {info['cpu_count_logical']} logical cores")
    logger.info(f"Memory: {info['memory_total_gb']} GB total, {info['memory_available_gb']} GB available")
    if info['gpu_count'] > 0:
...
def performance_monitor(operation_name: str = "Operation", log_memory: bool = True):
    ...
    if start_memory:
        logger.debug(f"Initial memory: {start_memory['used_gb']:.1f}GB ({start_memory['percent']:.1f}%)")
    ...
    try:
        ...
        if log_memory and start_memory:
            ...
            logger.debug(f"Final memory: {end_memory['used_gb']:.1f}GB ({end_memory['percent']:.1f}%)")
