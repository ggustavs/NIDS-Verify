...
class NIDSTrainer:
    ...
    def train(
        ...
        if training_type == "adversarial":
            ...
            if attack_rects is None:
                logger.info(
                    "Using research-based hyperrectangle definitions from original NIDS paper"
                )
            else:
        ...
        logger.info(
            f"Starting {training_type} training for {epochs} epochs, {steps_per_epoch} steps per epoch"
        )
    ...
    def _base_train_step(self, x_batch: tf.Tensor, y_batch: tf.Tensor) -> dict[str, float]:
        ...
        accuracy = tf.reduce_mean(
            tf.cast(
                tf.equal(tf.argmax(predictions, axis=1), tf.cast(y_batch, tf.int64)), tf.float32
            )
        )

        return {"loss": float(loss), "accuracy": float(accuracy), "gradients": gradients}
    ...
    def _adversarial_train_step(
        ...
        accuracy = tf.reduce_mean(
            tf.cast(
                tf.equal(tf.argmax(predictions, axis=1), tf.cast(y_mixed, tf.int64)), tf.float32
            )
        )
    ...
    def _analyze_gradients(self, gradients, step: int = 0, epoch: int = 0) -> dict[str, float]:
        ...
        return {"grad_global_norm": global_norm, "grad_status": status}
        ...
        try:
            ...
            model_metadata = {
                ...
                "trainable_parameters": sum(
                    [tf.size(var).numpy() for var in self.model.trainable_variables]
                ),
            ...
            logger.info("✅ Model successfully saved and registered:")
            logger.info("   - MLflow artifacts: model/")
...
def train_adversarial(
    ...
    return trainer.train(
        train_dataset, val_dataset, "adversarial", attack_rects, epochs, steps_per_epoch
    )
